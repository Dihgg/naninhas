...
export abstract class CommandHandler<TAuthoritative, TRequestPayload, TResponsePayload> {
	 ...
	 */
	public handler(props: { module: string; command: string; player: IsoPlayer; args: unknown }) {
		...
		const revisionsByCommand = protocol.lastClientRevisionByCommand ?? {};
		protocol.lastClientRevisionByCommand = revisionsByCommand;
		let lastCommandRevision = revisionsByCommand[this.command.REQUEST] ?? 0;

		if (payload.revision === 1 && lastCommandRevision > 0) {
			lastCommandRevision = 0;
			revisionsByCommand[this.command.REQUEST] = 0;
		}

		if (payload.revision <= lastCommandRevision) {
			print(
				`[${this.module}][Server][${this.command.REQUEST}] Ignoring stale or out-of-order request from player ${player.getUsername()}; incomingRevision=${payload.revision}; previousRevision=${lastCommandRevision}`
			);
			this.onStaleCommand(player, payload);
			return;
		}

		revisionsByCommand[this.command.REQUEST] = payload.revision;
		// Continue writing the legacy value so downgrades retain useful state.
	...
	private ensureServerModData(
		...
		return {
			protocol: {
				...
				// A legacy aggregate revision cannot safely be attributed to
				// either independent request stream, so migration starts both fresh.
				lastClientRevisionByCommand: data.protocol?.lastClientRevisionByCommand ?? {},
				lastSchemaVersion
			},
			authoritative:
				this.migrateAuthoritativeData(lastSchemaVersion, data.authoritative) ??
				this.defaultData
		};
	}
	...
	protected sendResponse(
		player: IsoPlayer,
		payload: CommandPayload<TRequestPayload>,
	...
	protected onUnsupportedSchema(
		_player: IsoPlayer,
		_payload: CommandPayload<TRequestPayload>
	): void {
